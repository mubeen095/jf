...
jQuery(window).load(function() {
	...
	jQuery('#main-nav').on("click", "li > a", function() {
		...
		if (jQuery('#menu').is('.menu-is-open')) {
			jQuery('#menu').toggleClass('menu-is-open');
		}
	});
	...
	jQuery('body').on('click', '#backtotop,#scroll-down,.scroll-to', function() {
		...
		jQuery('html,body').animate({ scrollTop: topPos - 60 }, 1000, 'easeInOutQuart');
		return false;
	});
	...
	if(jQuery().lightlayer) {
        jQuery('.ll-btn').click(function() {
        	var data_id = jQuery(this).data('id');
            $.lightlayer({
                object: jQuery('div#' + data_id),
				...
				onOpen: function( e, popup ) {
					...
					jQuery('form#' + data_id).parsley();
        			jQuery('form#' + data_id).submit(function(event){
            ...
            grecaptcha.reset();
            grecaptcha.render(document.getElementById(data_id + '-captcha'), {
	          'sitekey' : '6LcrUh0UAAAAAJCBuH5se1GPi2hJgX6SbzagbYOE',
	          'data-size' : 'compact',
	          'theme' : 'light',
    ...
    jQuery('form#comments-form').submit(function(event){
	    event.preventDefault();
	    submitForm(jQuery(this));
	});
    ...
    function submitForm(form) {
	    ...
	    var form = form;
	    var formMessages = jQuery(form).siblings('#form-messages');
	    ...
	    .done(function(response) {
	        ...
	        jQuery('#email').val('');
	        jQuery('#dogsname').val('');
	        jQuery('#checkindate').val('');
	        jQuery('#checkoutdate').val('');
	        jQuery('#dogbath').val('');
	        jQuery('#additionalcomments').val('');*/
	...
	if(jQuery().lightcase) { 
		...
		jQuery('a.donatebutton').lightcase({ 
			showTitle: false,
			showCaption: false,
			showSequenceInfo: false,
			overlayOpacity: 0.9,
		...
		/*jQuery('a.resbutton').lightcase({
		    ...
		    onFinish : {
		        baz: function() {
		            ...
		            jQuery.ajax({
		            	method: "GET",
		            	url: "files/js/reservations-app.js",
						dataType: "script"
					});
		        }
		    },
		...
		jQuery('a.trainbutton').lightcase({
			showTitle: false,
			showCaption: false,
			showSequenceInfo: false,
			overlayOpacity: 0.9,
...
jQuery(document).ready(function() { 
	...
	if( jQuery(".instagram-widget").length > 0  && jQuery().spectragram){
		jQuery.fn.spectragram.accessData = {
			accessToken: '5081682.b4a737c.f2d3b015723341779ba25b6ab8a0102c',
			clientID: 'b4a737c06e8f4b7083b298b033ec9153'
		};
